...
export async function POST(req: Request) {
  try {
    ...
    const { name, level, archetype, xp, topLanguage, medals } = data;

    const defaultPost = `I just discovered my GitHub Developer Identity on Git DNA! 🧬\n\nI'm a Level ${level} "${archetype}" with ${xp} XP. My primary focus is ${topLanguage}, and I've unlocked ${medals} developer achievements.\n\nCheck out your own Git DNA today! #GitDNA #Developer #GitHub`;
    ...
    const prompt = `
      You are an expert social media manager writing a fun, slightly edgy, and engaging LinkedIn post for a developer.
      ...
      Write a short, engaging LinkedIn post (under 400 characters) that they can share.
      Make it sound proud but not arrogant. 
